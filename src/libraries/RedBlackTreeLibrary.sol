...
library RedBlackTreeLibrary {
    ...
    function next(
        Tree storage self,
        Price target
    ) internal view returns (Price cursor) {
        if (isEmpty(target)) {
        ...
        if (isNotEmpty(self.nodes[target].right)) {
            ...
            while (
                isNotEmpty(cursor) &&
                Price.unwrap(target) == Price.unwrap(self.nodes[cursor].right)
            ) {
                target = cursor;
    ...
    function prev(
        ...
        if (isNotEmpty(self.nodes[target].left)) {
            ...
            while (
                isNotEmpty(cursor) &&
                Price.unwrap(target) == Price.unwrap(self.nodes[cursor].left)
            ) {
                target = cursor;
                cursor = self.nodes[cursor].parent;
            }
    ...
    function exists(Tree storage self, Price key) internal view returns (bool) {
        return
            ...
            ((Price.unwrap(key) == Price.unwrap(self.root)) ||
                isNotEmpty(self.nodes[key].parent));
    }
    ...
    function remove(Tree storage self, Price key) internal {
        ...
        if (isNotEmpty(yParent)) {
            if (
                Price.unwrap(cursor) == Price.unwrap(self.nodes[yParent].left)
            ) {
    ...
    function rotateLeft(Tree storage self, Price key) private {
        ...
        if (isEmpty(keyParent)) {
            ...
            Price.unwrap(key) == Price.unwrap(self.nodes[keyParent].left)
        ) {
    ...
    function rotateRight(Tree storage self, Price key) private {
        ...
        if (isEmpty(keyParent)) {
            ...
            Price.unwrap(key) == Price.unwrap(self.nodes[keyParent].right)
        ) {
            self.nodes[keyParent].right = cursor;
        } else {
            self.nodes[keyParent].left = cursor;
        ...
        while (
            ...
            if (
                ...
                if (self.nodes[cursor].red == RED_TRUE) {
                    ...
                    if (
                        Price.unwrap(key) ==
                        Price.unwrap(self.nodes[keyParent].right)
                    ) {
                        key = keyParent;
                        rotateLeft(self, key);
                    }
                ...
                if (self.nodes[cursor].red == RED_TRUE) {
                    ...
                    if (
                        Price.unwrap(key) ==
                        Price.unwrap(self.nodes[keyParent].left)
                    ) {
                        key = keyParent;
